...
public class VideoPostRepositoryWithBagRelationshipsImpl implements VideoPostRepositoryWithBagRelationships {
    ...
    public Optional<VideoPost> fetchBagRelationships(Optional<VideoPost> videoPost) {
        return videoPost.map(this::fetchTags).map(this::fetchAffinityVectors);
    }
    ...
    public List<VideoPost> fetchBagRelationships(List<VideoPost> videoPosts) {
        return Optional.of(videoPosts).map(this::fetchTags).map(this::fetchAffinityVectors).orElse(Collections.emptyList());
    }
    ...
    VideoPost fetchAffinityVectors(VideoPost result) {
        return entityManager
            .createQuery(
                "select videoPost from VideoPost videoPost left join fetch videoPost.affinityVectors where videoPost.id = :id",
                VideoPost.class
            )
            .setParameter(ID_PARAMETER, result.getId())
            .getSingleResult();
    ...
    List<VideoPost> fetchAffinityVectors(List<VideoPost> videoPosts) {
        HashMap<Object, Integer> order = new HashMap<>();
        IntStream.range(0, videoPosts.size()).forEach(index -> order.put(videoPosts.get(index).getId(), index));
        List<VideoPost> result = entityManager
            .createQuery(
                "select videoPost from VideoPost videoPost left join fetch videoPost.affinityVectors where videoPost in :videoPosts",
                VideoPost.class
            )
            .setParameter(VIDEOPOSTS_PARAMETER, videoPosts)
            .getResultList();
