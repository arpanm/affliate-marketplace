...
public class VideoPostResource {
    ...
    public List<VideoPost> getAllVideoPosts(
        ...
        if ("competitionwinner-is-null".equals(filter)) {
            LOG.debug("REST request to get all VideoPosts where competitionWinner is null");
            return StreamSupport.stream(videoPostRepository.findAll().spliterator(), false)
                .filter(videoPost -> videoPost.getCompetitionWinner() == null)
                .toList();
        }
