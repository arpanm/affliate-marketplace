...
public class VideoUserResource {
     ...
     */
    @GetMapping("")
    public List<VideoUser> getAllVideoUsers(@RequestParam(name = "eagerload", required = false, defaultValue = "true") boolean eagerload) {
        LOG.debug("REST request to get all VideoUsers");
        if (eagerload) {
            return videoUserRepository.findAllWithEagerRelationships();
        } else {
    ...
    public ResponseEntity<VideoUser> getVideoUser(@PathVariable("id") Long id) {
        ...
        Optional<VideoUser> videoUser = videoUserRepository.findOneWithEagerRelationships(id);
