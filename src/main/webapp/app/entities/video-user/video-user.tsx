          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="affliateMarketplaceApp.videoUser.bank">Bank</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="affliateMarketplaceApp.videoUser.affinityVectors">Affinity Vectors</Translate>{' '}
                  <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="affliateMarketplaceApp.videoUser.company">Company</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {videoUserList.map((videoUser, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{videoUser.bank ? <Link to={`/bank-details/${videoUser.bank.id}`}>{videoUser.bank.id}</Link> : ''}</td>
                  <td>
                    {videoUser.affinityVectors
                      ? videoUser.affinityVectors.map((val, j) => (
                          <span key={j}>
                            <Link to={`/affinity/${val.id}`}>{val.id}</Link>
                            {j === videoUser.affinityVectors.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>{videoUser.company ? <Link to={`/sponsor/${videoUser.company.id}`}>{videoUser.company.id}</Link> : ''}</td>
